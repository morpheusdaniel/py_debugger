...
class debugger():
	...
	def open_process(self, pid):
		...
		#DWORD ppid = kernel32.GetWindowThreadProcessId(wnd,pid);
		h_process = kernel32.OpenProcess(PROCESS_ALL_ACCESS,False,pid)
		#h_process = kernel32.OpenProcess(MAXIMUM_ALLOWED,False,pid)
		return h_process
	...
	def exception_handler_breakpoint(self):
		...
		if self.first_breakpoint:
			...
			try:
				...
				print "[*] breakpoint removed: 0x%08x" % self.exception_address
			except:
				print "yes you failed"
				return DBG_CONTINUE
	...
	def bp_set_hw(self, address, length, condition):
		...
		if not self.hardware_breakpoints.has_key(0):
			...
			available = 2
		elif not self.hardware_breakpoints.has.key(3):
			available = 3
		else:
			return False
		...
		print "[*] Setting hw breakpoint at: 0x%08x" % address
		return True
	...
	def bp_set_mem(self, address, size):
		...
		print "[*] Setting memory breakpoint at: 0x%08x * 0x%08x" % (address, size)
		return True
